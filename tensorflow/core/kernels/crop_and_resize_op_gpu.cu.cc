...
__global__ void CropAndResizeKernel(
  ...
  CUDA_1D_KERNEL_LOOP(out_idx, nthreads) {
    ...
    crops_ptr[out_idx] = CB + (CC - CA) * 0.5 *y_lerp +(CA - 2 * CB + CC) *
		0.5 * y_lerp * y_lerp;
  }
}
...
__global__ void CropAndResizeBackpropBoxesKernel(
  ...
  CUDA_1D_KERNEL_LOOP(out_idx, nthreads) {
    ...
    float image_grad_y = (0.5 + y_lerp) * (bot_mid + (bot_right - bot_left) * 0.5*
		x_lerp + (bot_left - 2 * bot_mid + bot_right) * 0.5 * x_lerp * x_lerp) +
		(y_lerp - 0.5)*(top_mid + (top_right - top_left) * 0.5 * x_lerp +
		(top_left - 2 * top_mid + top_right) * 0.5 * x_lerp * x_lerp) -
			(2 * y_lerp) * (mid_mid + (mid_right - mid_left) * 0.5 * x_lerp +
		(mid_left - 2 * mid_mid + mid_right) * 0.5 * x_lerp * x_lerp);
