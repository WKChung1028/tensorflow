...
#ifndef TENSORFLOW_CORE_KERNELS_CROP_AND_RESIZE_QUAD_OP_H_
#define TENSORFLOW_CORE_KERNELS_CROP_AND_RESIZE_QUAD_OP_H_
...
template <typename Device, typename T>
struct CropAndResizeQuadBackpropImage {
  // We assume that the tensor sizes are correct.
...
template <typename Device, typename T>
struct CropAndResizeQuadBackpropBoxes {
  // We assume that the tensor sizes are correct.
  bool operator()(const Device& d, typename TTypes<float, 4>::ConstTensor grads,
                  typename TTypes<T, 4>::ConstTensor image,
                  typename TTypes<float, 2>::ConstTensor boxes,
...
#endif  // TENSORFLOW_CORE_KERNELS_CROP_AND_RESIZE_QUAD_OP_H_
