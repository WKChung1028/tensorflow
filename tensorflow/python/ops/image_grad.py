...
@ops.RegisterGradient("CropAndResizeQuad")
def _CropAndResizeQuadGrad(op, grad):
  """The derivatives for crop_and_resize.

  We back-propagate to the image only when the input image tensor has floating
  point dtype but we always back-propagate to the input boxes tensor.
  Mainly same as  CropAndResize.

  Args:
    op: The CropAndResizeQuad op.
    grad: The tensor representing the gradient w.r.t. the output.
  ...
  if op.inputs[0].dtype in allowed_types:
    # pylint: disable=protected-access
    grad0 = gen_image_ops.crop_and_resize_quad_grad_image(grad,
                                                     op.inputs[1],
                                                     op.inputs[2],
                                                     image_shape,
                                                     T=op.get_attr("T"))
  ...
  grad1 = gen_image_ops.crop_and_resize_quad_grad_boxes(grad, op.inputs[0],
                                                   op.inputs[1], op.inputs[2])
